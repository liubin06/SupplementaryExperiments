...
project_data= r'F:\desktop\data\5f\100k.csv'
train_data = r'F:\desktop\data\5f\100k_train.csv'
test_data = r'F:\desktop\data\5f\100k_test.csv'
...
def sofmax(pos_score,neg_socre):
    e_x = np.exp(pos_score)
    e_y = np.exp(neg_socre)
    prob = e_x / (e_x+e_y)
    return prob
    ...
    def train(self, data_pair):
        for epoch in range(1, self.train_count + 1):
            ...
            for ui in data_pair:
                ...
                xui = np.dot(self.U[int(u)],self.V[int(i)])
                loss_func1 = 1-sigmoid(xui)
                self.U[int(u)] += self.lr * (loss_func1 * self.V[int(i)]  - self.reg * self.U[int(u)])
                self.V[int(i)] += self.lr * (loss_func1 * self.U[int(u)] - self.reg * self.V[int(i)])
                ...
                #j = ns.rns(negative_items)
                xuj = rating_vector[int(j)]
                loss_func2 = sigmoid(xuj)
                self.U[int(u)] += self.lr * (loss_func2 * ( - self.V[int(j)]) - self.reg * self.U[int(u)])
                self.V[int(j)] += self.lr * (loss_func2 * (- self.U[int(u)]) - self.reg * self.V[int(j)])
