...
def rns(negative_items):
    ...
    j = np.random.choice(negative_items, size=1, replace=False)[0]  #O(|I|)
    return j
...
def bns(i, negative_items, negative_index, rating_vector,prior, size, alpha):
    ...
    F_n = np.array([np.sum(rating_vector <= x_ul) / 1683 for x_ul in candidate_scores])   #O(|I|)
